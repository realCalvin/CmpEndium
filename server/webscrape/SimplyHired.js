...
async function simplyHiredScraper(jobInfo) {
    ...
    let simplyHiredLink =
    'https://www.simplyhired.com/jobs?q=' + keywords + '&l=' + location;
    ...
    await puppeteer
        ...
        .then(async (browser) => {
            ...
            for (let N = 1; N < 2; N++) {
                ...
                await page
                    .goto(currentSimplyHiredLink, { waitUntil: 'domcontentloaded' })
                    .then(async () => {
                        ...
                        await content.then((success) => {
                            ...
                            $('#job-list .SerpJob-jobCard').each(function() {
                                if (
                                    $(this).find(
                                        '.SerpJob-metaInfoLeft .SerpJob-simplyApplyLabel'
                                    ).length === 0
                                ) {
                                    simplyHiredJobs.push({
                                        ...
                                        location: $(this)
                                            .find('.jobposting-location .jobposting-location')
                                            .text()
                                            .trim(),
                                        link:
                      'https://www.simplyhired.com' +
                      $(this).find('.jobposting-title a').attr('href')
                                    });
                                }
                            });
                        });
            ...
            await (async () => {
                ...
                await cluster.task(async ({ page, data }) => {
                    ...
                    await page
                        .goto(data.url, { waitUntil: 'domcontentloaded' })
                        .then(async () => {
                            const content = page.content();
                            await content.then((success) => {
                                const $ = cheerio.load(success);
                                // Retrieve External Job Link and Job Description
                                simplyHiredJobs[data.pos].link =
                  'https://www.simplyhired.com' +
                  $('.viewjob-buttons .apply a').attr('href');
                                simplyHiredJobs[data.pos].description = $(
                ...
                for (let i = 0; i < simplyHiredJobs.length; i++) {
